...
for ($i = 0; $i -lt 2; $i++) {
    ...
    New-VM -Name $vmName -Memory 6GB -Path $vmPathloc -Generation 2
    ...
    Add-VMNetworkAdapter -VMName $vmName -SwitchName "vsw-TOR1" -Name "LAN2"
    Set-VMNetworkAdapter -VMName $vmName -MacAddressSpoofing On -Name "LAN2"
    Set-VM -VMName $vmName -ProcessorCount 4
    New-Item -Path "$vmPathloc\Virtual Hard Disks" -ItemType Directory -Force
    New-VHD -Path "$vmPathloc\Virtual Hard Disks\$vmName.vhdx" -Size 32GB -Dynamic -BlockSizeBytes 1MB
